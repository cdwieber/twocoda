...
if ( ! function_exists( 'et_setup_theme' ) ){
	function et_setup_theme(){
		...
		require_once( $template_directory . '/includes/post_thumbnails_fusion.php' );
...
function et_fusion_load_scripts_styles(){
	...
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'Fusion' ) && et_core_use_google_fonts() ) {
...
function et_add_standard_homepage_class( $classes ) {
	...
	if ( is_home() && 'on' == et_get_option( 'fusion_featured', 'on' ) ) $classes[] = 'et_featured_slider';
