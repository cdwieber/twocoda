...
class phpinvoice extends FPDF_rotation  {
	...
	private $font 			 = 'DejaVu';	 	/* Font Name : See inc/fpdf/font for all supported fonts */
    ...
    ******************************************/
	...
	private function setLanguage($language) {
		...
		$lang = array();
		$lang['number']		= __('Reference', 'wp-ultimo');
		$lang['date'] 		= __('Billing Date', 'wp-ultimo');
		$lang['time']			= __('Billing Time', 'wp-ultimo');
		$lang['due'] 			= __('Due Date', 'wp-ultimo');
		$lang['to'] 			= __('Billing To', 'wp-ultimo');
		$lang['from'] 		= __('Billing From', 'wp-ultimo');
		$lang['product'] 	= __('Product', 'wp-ultimo');
		$lang['qty'] 			= __('Qty', 'wp-ultimo');
		$lang['price'] 	  = __('Price', 'wp-ultimo');
		$lang['discount'] = __('Discount', 'wp-ultimo');
		$lang['vat'] 			= __('Vat', 'wp-ultimo');
		$lang['total'] 	  = __('Total', 'wp-ultimo');
		$lang['page'] 		= __('Page', 'wp-ultimo');
		$lang['page_of'] 	= __('of', 'wp-ultimo');
	...
	public function Header() {
		...
		if(!empty($this->reference)) {
		  ...
		  $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		  $this->Cell(32,$lineheight,strtoupper($this->lang['number']).':',0,0,'L');
		...
		$this->SetFont($this->font,'B',9);
		$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		$this->Cell(32,$lineheight,strtoupper($this->lang['date']).':',0,0,'L');	
		$this->SetTextColor(50,50,50);
		$this->SetFont($this->font,'',9);
		...
		if(!empty($this->time)){
		  ...
		  $this->SetFont($this->font,'B',9);
		  $this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
		  $this->Cell(32,$lineheight,strtoupper($this->lang['time']).':',0,0,'L');	
		  $this->SetTextColor(50,50,50);
		  $this->SetFont($this->font,'',9);
		...
		if(!empty($this->due)){
			...
			$this->SetFont($this->font,'B',9);
			$this->SetTextColor($this->color[0],$this->color[1],$this->color[2]);
			$this->Cell(32,$lineheight,strtoupper($this->lang['due']).':',0,0,'L');	
			$this->SetTextColor(50,50,50);
			$this->SetFont($this->font,'',9);
		...
		if(!isset($this->productsEnded))  {
			...
			$this->Cell($this->firstColumnWidth,10,strtoupper($this->lang['product']),0,0,'L',0);
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,strtoupper($this->lang['qty']),0,0,'C',0);
			if(isset($this->vatField) and $this->hide_vat == 0) {
				$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
				$this->Cell($width_other,10,strtoupper($this->lang['vat']),0,0,'C',0);
			}
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,strtoupper($this->lang['price']),0,0,'C',0);
			if(isset($this->discountField) and $this->hide_discount == 0) {
				$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
				$this->Cell($width_other,10,strtoupper($this->lang['discount']),0,0,'C',0);
			}
			$this->Cell($this->columnSpacing,10,'',0,0,'L',0);
			$this->Cell($width_other,10,strtoupper($this->lang['total']),0,0,'C',0);
			$this->Ln();
	...
	public function Body() {	
		...
		if($this->items) {
			foreach($this->items as $item) 
			{
				...
				if($this->curreny_direction == "left") {
					$this->Cell($width_other,$cHeight,$this->currency.' '.number_format($item['price'],2,$this->referenceformat[0],$this->referenceformat[1]),0,0,'C',1);
				}else{
					$this->Cell($width_other,$cHeight,number_format($item['price'],2,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency,0,0,'C',1);
				}
				...
				if($this->curreny_direction == "left") {
				$this->Cell($width_other,$cHeight,$this->currency.' '.number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1]),0,0,'C',1);
				}else{
				$this->Cell($width_other,$cHeight,number_format($item['total'],2,$this->referenceformat[0],$this->referenceformat[1]).' '.$this->currency,0,0,'C',1);				
				}
		...
		foreach($this->addText as $text)  {
			...
			if($text[0] == 'paragraph') 
			{
				...
				$this->SetFont($this->font,'',8);
				$this->MultiCell(0,4,$this->WriteHTML($text[1]),0,'L',0);
				$this->Ln(4);
			}
		}
	...
	public function Footer() {
		...
		$this->Cell(0,10,$this->WriteHTML($this->footernote),0,0,'L');
