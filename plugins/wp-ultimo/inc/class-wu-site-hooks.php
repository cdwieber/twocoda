...
class WU_Site_Hooks {
  ...
  public function __construct() {
    ...
    add_action('delete_blog', array($this, 'on_delete_blog'));

    // Clears site template caches on delete blog
    add_action('delete_blog', array($this, 'clear_available_site_template_cache'));
  ...
  public function set_cors_domain_for_template_preview() {
    ...
    echo "<script type='text/javascript'>document.domain = '{$current_site->domain}';</script>";
  ...
  public function add_switching_templates_page() {
    ...
    add_submenu_page('none', __('Switching Templates', 'wp-ultimo'), __('Switching Templates', 'wp-ultimo'), 'read', 'wu-new-template', array($this, 'switching_templates'), false);
  ...
  public function new_site_menu() {
    ...
    if ((!is_main_site() && $subscription && WU_Settings::get_setting('enable_multiple_sites')) || $needs_site) {
      ...
      $this->new_site_hook = add_submenu_page('index.php', __('Add New Site', 'wp-ultimo'), __('Add New Site', 'wp-ultimo'), 'read', 'wu-new-site', array($this, 'new_site_for_user_form'), false);
  ...
  public function save_add_plan($user_id) {
    ...
    if ($plan->getPlan($_POST['subscription_plan'])) {
