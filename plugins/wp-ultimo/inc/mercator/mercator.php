...
/**
 * Returns the SSO enable status
 * @since 1.5.0
 * @return bool
 */
function wu_check_sso() {
	
	$settings = get_network_option(null, 'wp-ultimo_settings');

	return isset($settings['enable_sso']) ? $settings['enable_sso'] : true;

}

// Handles SSO
add_filter('mercator.sso.enabled', __NAMESPACE__ . '\\wu_check_sso');
...
/**
 * Bypass brizy mapped domain filtering plugins_url before hooks.
 *
 * @since 1.10.0
 *
 * @return void
 */
function bypass_brizy() {
 ...
 */
function mangle_url( $url, $path, $orig_scheme = 'http', $site_id = 0 ) {
	if ( empty( $site_id ) ) {
