...
class WU_Widgets {
  ...
  public static function output_widget_news($user) { ?>
    ...
    <div class="rss-widget">
      <div class='wu-rss-widget-title'><?php  _e('Community Forums', 'wp-ultimo'); ?></div>
      <div id='wp-ultimo-forum-feed'><?php _e('Loading...', 'wp-ultimo'); ?></div>
    </div>
    ...
    <div id="major-publishing-actions" style="margin: 12px -12px -12px;">
      <a target="_blank" href="<?php echo WU_Links()->get_link('forums'); ?>" class="button button-primary button-streched"><?php _e('Join WP Ultimo Forums', 'wp-ultimo'); ?> &rarr;</a>
    </div>
        ...
        $(document).ready(function() {
          ...
          $.get({
            ...
            success: function(response) {
              $('#wp-ultimo-forum-feed').html(response);
            },
            error: function() {
              $('#wp-ultimo-forum-feed').html('<?php echo __("Error loading external feed.", "wp-ultimo"); ?>');
            }
          }) // end get;
