...
class WU_Gateway_Stripe extends WU_Gateway {
  ...
  public function get_current_payment_method($customer_id) {
    ...
    try {
      ...
      if ($payment_method->default_source->object == 'card') {
  ...
  public function change_stripe_button($html) {
    ...
    <form action="<?php echo $this->get_url('process-integration'); ?>" method="POST">
            
      <noscript><?php printf('You must <a href="%s" target="_blank">enable JavaScript</a> in your web browser in order to pay via Stripe.', 'http://www.enable-javascript.com'); ?></noscript>

      <script type="text/javascript" src="https://checkout.stripe.com/checkout.js"></script>

      <input 
        id = "stripe-button" 
        class="button button-primary button-streched button-gateway" 
        style="width: 100%; text-align: center; margin-top: 10px;" 
        type="submit" 
        data-image="<?php echo WU_Settings::get_logo('full', WU_Settings::get_setting('logo-square')); ?>" 
        value="<?php echo $this->get_button_label(); ?>" 
        data-key="<?php echo WU_Settings::get_setting('stripe_api_pk'); ?>" 
        data-amount="<?php echo (float) $this->format_for_stripe( $value ); ?>" 
        data-currency="<?php echo WU_Settings::get_setting('currency_symbol') ?>" 
        data-name="<?php echo $this->plan->title; ?>" 
        data-description="<?php echo $desc; ?>" 
        data-email="<?php echo $this->site->site_owner->user_email; ?>" 
        data-allow-remember-me="false"
        data-locale="<?php echo strtok(get_user_locale($this->subscription->user_id), '_'); ?>" 
        data-zip-code="<?php echo WU_Settings::get_setting('stripe_should_collect_billing_address', false) ? 'true' : 'false' ?>" 
        data-billing-address="<?php echo WU_Settings::get_setting('stripe_should_collect_billing_address', false) ? 'true' : 'false' ?>" 
