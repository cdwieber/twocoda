...
if( !class_exists( 'MUCD_Data' ) ) {
    ...
    class MUCD_Data {
         ...
         */
        public static function copy_data( $from_site_id, $to_site_id, $replace_media_url = true) {
            self::$to_site_id = $to_site_id;

            // Copy
            MUCD_Data::db_copy_tables( $from_site_id, $to_site_id );
            // Update
            MUCD_Data::db_update_data( $from_site_id, $to_site_id, $replace_media_url);
        }
        ...
        public static function db_copy_tables( $from_site_id, $to_site_id ) {
            ...
            if($from_site_id == MUCD_PRIMARY_SITE_ID) {
                ...
                // @since 1.6.2
                $from_site_table =  apply_filters('wu_filter_tables_to_copy', MUCD_Data::do_sql_query($sql_query, 'col')); 
            }
       ...
       public static function get_primary_tables($from_site_prefix) {
         ...
         * @param  int $to_site_id   new site id
         */
        public static function db_update_data( $from_site_id, $to_site_id, $replace_media_url = true) {
            ...
            foreach( $tables as $table => $field) {
                 ...
                 * Replace Media
                 */
                if ($replace_media_url) {
                    MUCD_Data::update($to_blog_prefix . $table, $field, $from_upload_url, $to_upload_url);
                }
        ...
        public static function try_replace( $row, $field, $from_string, $to_string) {
            if (is_serialized($row[$field])) {
                ...
                if (is_array($row[$field])) {
                    ...
                    // foreach($array_object as $key => $field) {
                    $row[$field] = (object) $array_object; // FIx for deep serialization like Beaver Builder
                    // }
                }
                else {
                    $row[$field] = MUCD_Data::replace($row[$field], $from_string, $to_string);
        ...
        public static function sql_error($sql_query, $sql_error) {
            ...
            if( $log_url = MUCD_Duplicate::log_url() ) {
                echo '<a href="' . $log_url . '">' . WU_NETWORK_PAGE_DUPLICATE_VIEW_LOG . '</a>';
            }
