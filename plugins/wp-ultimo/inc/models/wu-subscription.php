...
class WU_Subscription {
  ...
  public function get_coupon_code_string() {
    ...
    $message = __('%s - Coupon code of <b>%s on Subscription</b>, for %s.', 'wp-ultimo');

    if ($coupon_code['applies_to_setup_fee'] && $coupon_code['setup_fee_discount_value']) {

      $message = __('%s - Coupon code of <b>%s on Subscription</b> and <b>%s on Setup Fee</b>, for %s.', 'wp-ultimo');
    ...
    return sprintf($message, $coupon_code['coupon_code'], $value, $setup_fee_value, $cycles);
  ...
  public function get_price_after_coupon_code() {
    ...
    return apply_filters('wu_subscription_get_price_after_coupon_code', round($value, 2), $this);
