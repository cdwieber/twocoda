    ...
    <div class="wu-col-sm-10">
      ...
      <div class="">
        <label class="description">
          <input v-model="send_payment" <?php disabled( !$g || !$g->supports_single_payments() ); ?> type="checkbox" name="remote_payment" value="1">
          <?php _e('Charge the Integrated Payment Method.', 'wp-ultimo'); ?> 
          <?php echo WU_Util::tooltip(__('This will create a real charge on the integrated payment method (charge the user card, for example).', 'wp-ultimo')); ?>
          <?php echo !$g || !$g->supports_single_payments() ? __('(The current payment integration does not support this option)', 'wp-ultimo') : ''; ?> 
        </label>
      </div>
